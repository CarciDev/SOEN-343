...
import { checkoutDataSchema, CheckoutPaymentAdapter } from '$lib/domain/CheckoutPaymentAdapter';
...
export const GET: RequestHandler = async (event) => {
  ...
  const quotationPriceInCents = parseInt(event.url.searchParams.get('amountQuotedCents') || '0', 10);
  ...
  try {
    ...
    checkoutUrl = await createQuotationCheckoutSession(validatedStripeCheckoutData);
  } catch (e) {
